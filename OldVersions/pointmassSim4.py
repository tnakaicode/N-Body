...
class Mass(object): #Mass template object
    def __init__(self):
        ...
        # SET POSITION
        self.Xposition = rand.uniform(-150,150)  #generate X position; random number between 0,1
        self.Yposition = rand.uniform(-150,150)  #generate Y position; random number between 0,1
        self.Zposition = rand.uniform(-150,150)  #generate Z position; random number between 0,1
    ...
    def CalcAccel(self):   #CALCULATE ACCERATION DUE TO OTHER OBJECTS
        ...
        for massindex in range(masses):  #go through each objec to find accel from that object
            ...
            if radius > .7:        #only continue if there is a difference in radius
                self.totalaccel = self.totalaccel + nam.mass/(radius**2) #find the total amount of acceleration
    ...
    def InitVelo(self):
        ...
        vdirect = -(self.Xacceleration/self.Yacceleration) #calculate the direction of the velocity vector if perp to the radial vector in xy plane
        totalvelocity = math.sqrt(xyrad*self.totalaccel) #calculate velocity needed given radius and acceleration (v**2/r = a)

        # SET VELOCITY
        self.Xvelocity = totalvelocity/(math.sqrt(vdirect**2+1)) #velo in x direction given by total velo/ (slope^2+1)
        self.Yvelocity = self.Xvelocity*vdirect #use slope to calculate velo in y direction
    ...
    def CheckBoundries(self, Xposition,Yposition,Zposition): #reverse velocity if position exceeds boundries
