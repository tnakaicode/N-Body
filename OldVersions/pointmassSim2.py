...
class Mass(object): #Mass template object
    def __init__(self, name):
    	...
    	self.Xposition = rand.uniform(-10,10)  #generate X position random number between 0,1
    	self.Yposition = rand.uniform(-10,10)  #generate Y position random number between 0,1
    ...
    def CalcAccel(self): #calculate accel on object
    	for massindex in range(masses):  #go through each objec to find accel from that object
    		...
    		radius = math.sqrt((Xdiff)**2 + (Ydiff)**2)  #find the distance to the other particle
    		if radius != 0:        #only continue if there is a difference in radius
    			totalaccel = nam.mass/(radius**2)
    			self.Xacceleration =  totalaccel*Xdiff/(radius) # create Xacceleration
    			self.Yacceleration =  totalaccel*Ydiff/(radius) # create Xacceleration
...
#		print 'oldpos' + str(objlist[i].position)
		...
		objlist[i].CalcPos(objlist[i].Xvelocity,objlist[i].Yvelocity,objlist[i].Xacceleration, objlist[i].Yacceleration)  #calculate position
#		print 'newpos:' + str(objlist[i].position)

		objlist[i].CalcVelo(objlist[i].Xacceleration,objlist[i].Yacceleration) #calculate velocity
